...
export default async function EditContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="max-w-4xl mx-auto p-4 sm:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              ...
              {/* C1: comic type for visual-strip publications routed to /dispatch/comics */}
              <option value="comic">Comic (Visual Strip)</option>
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="comics">Comics Archive</option>
            </select>
          </div>
        ...
        */}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-stone-200">
          ...
          <div className="flex items-center gap-4">
            {/*
              "Save Draft" persists field edits via POST /api/content/[id], which never
              touches `status` — so saving an already-published item keeps it published.
              All status changes (Submit/Approve/Publish/Unpublish) live in ReviewControls.
            */}
            <button
              type="submit"
              name="action"
            ...
            {/*
              Replaces the old "Update & Publish" button (which was confusing — the route
              never published, and real publishing belongs to ReviewControls). This renders
              the current editor content with the public renderers, working for drafts.
            */}
            <PreviewButton comicPanels={item.comic_panels} />
          </div>
        </div>
      </form>
      ...
      {/* Bottom back-to-dashboard affordance so editors don't have to scroll to the top. */}
