...
const HOME_DESCRIPTION =
  'A signal station at the edge of the internet. A small crew keeps one honest signal alive against the noise — local, true, and yours.'
...
export default function HomePage() {
  const db = getDb()
  // Latest Transmissions: the 3 most recent published pieces of ANY type (comic or multimedia),
  // not filtered to a single character. Card href routes comics to /dispatch/comics/<slug>.
  const latestItems = db.prepare(
    `SELECT id, slug, title, excerpt, series, character, created_at
     FROM content WHERE status = 'published' AND tier = 'free' ORDER BY created_at DESC LIMIT 3`
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
          <div className="flex flex-col gap-6 justify-center">
            <p className="text-body-md text-white/70">
              A signal station at the edge of the internet. A small crew keeps one honest signal
              alive against the noise — local, true, and yours.
            </p>
            <div>
              <a
                href="/about"
